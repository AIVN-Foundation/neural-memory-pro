...
from neural_memory_pro.infinitydb.wal import WAL_HEADER_SIZE, WAL_MAGIC, WALOp, WriteAheadLog
...
class TestWALLogging:
    ...
    @pytest.mark.asyncio
    async def test_add_neuron_with_embedding_logs_to_wal(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
    ...
    @pytest.mark.asyncio
    async def test_multiple_ops_accumulate_in_wal(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
...
class TestCrashRecovery:
    ...
    @pytest.mark.asyncio
    async def test_recover_add_neuron_with_embedding(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
        vec = _random_vec(dims)
...
class TestReplayIdempotency:
    ...
    @pytest.mark.asyncio
    async def test_replay_add_neuron_idempotent(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
    ...
    @pytest.mark.asyncio
    async def test_replay_add_synapse_idempotent(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
        await db.add_neuron("a", neuron_id="n1")
    ...
    @pytest.mark.asyncio
    async def test_replay_update_missing_neuron_skips(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
        await db.add_neuron("temp", neuron_id="n1")
...
class TestWALEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_wal_survives_open_close_cycle(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
    ...
    @pytest.mark.asyncio
    async def test_corrupt_wal_entry_stops_replay(
        self, db_dir: Path, dims: int
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_many_operations_then_recover(
        self, db_dir: Path, dims: int
    ) -> None:
