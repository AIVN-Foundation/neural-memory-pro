...
from neural_memory_pro.infinitydb.tier_manager import TierConfig
...
class TestInitialTier:
    ...
    @pytest.mark.asyncio
    async def test_high_priority_neuron_is_active(
...
class TestAutoPromote:
    ...
    @pytest.mark.asyncio
    async def test_promote_on_access_with_high_priority(
        self, db_dir: Path, dims: int
    ) -> None:
...
class TestDemoteSweep:
    ...
    async def test_demote_stale_neurons(self, db_dir: Path, dims: int) -> None:
        config = TierConfig(
            warm_after_days=7, cool_after_days=30, frozen_after_days=90
        )
        db = await _make_db(db_dir, dims, tier_config=config)
    ...
    @pytest.mark.asyncio
    async def test_no_demotion_for_fresh_neurons(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
    ...
    @pytest.mark.asyncio
    async def test_high_priority_never_demoted(
        self, db_dir: Path, dims: int
    ) -> None:
...
class TestTierStats:
    ...
    @pytest.mark.asyncio
    async def test_stats_reflect_tier_distribution(
        self, db_dir: Path, dims: int
    ) -> None:
        db = await _make_db(db_dir, dims)
