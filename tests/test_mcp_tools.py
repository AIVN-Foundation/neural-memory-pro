"""Tests for Pro MCP tool schemas and plugin registration."""

from __future__ import annotations

from neural_memory_pro.mcp_tools import PRO_TOOL_SCHEMAS, TOOL_HANDLERS
...
class TestPluginToolRegistration:
    def test_plugin_returns_tools(self) -> None:
        plugin = NMProPlugin()
        tools = plugin.get_tools()
        assert len(tools) == 3

    def test_plugin_returns_handlers(self) -> None:
        plugin = NMProPlugin()
        handler = plugin.get_tool_handler("nmem_cone_query")
        assert handler is not None
        assert callable(handler)

    def test_plugin_returns_none_for_unknown(self) -> None:
        plugin = NMProPlugin()
        handler = plugin.get_tool_handler("nmem_nonexistent")
