...
from neural_memory_pro.infinitydb.migrator import (
    ...
    *,
    ...
    for i in range(neurons):
        ...
        c.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (f"n{i}", f"neuron content {i}", "fact" if i % 2 == 0 else "decision",
             5 + i % 5, 1.0, 0, tags, emb_blob),
        )
...
class TestEstimateMigration:
    async def test_estimate_counts(self, tmp_path: Path) -> None:
        db_path = _create_sqlite_db(
            tmp_path / "test.db", neurons=10, synapses=5, fibers=2
        )
...
class TestMigratorBasic:
    async def test_migrate_missing_sqlite(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
        ...
        migrator = SQLiteToInfinityMigrator(
            tmp_path / "nonexistent.db", infinity_db
        )
    ...
    async def test_migrate_with_embeddings(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
        db_path = _create_sqlite_db(
            tmp_path / "src.db", neurons=3, embeddings=True, dimensions=8
        )
        await infinity_db.open()
    ...
    async def test_migrate_full(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
        ...
        db_path = _create_sqlite_db(
            tmp_path / "src.db",
            neurons=10, synapses=8, fibers=3,
            embeddings=True, dimensions=8,
        )
        await infinity_db.open()
...
class TestMigratorEdgeCases:
    ...
    async def test_neuron_without_id(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
        ...
        conn = sqlite3.connect(str(db_path))
        conn.execute("CREATE TABLE neurons (id TEXT, content TEXT, type TEXT DEFAULT 'fact', priority INTEGER DEFAULT 5)")
    ...
    async def test_synapse_without_source_or_target(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
        """Synapses with empty source/target should be skipped."""
        db_path = tmp_path / "bad_syn.db"
        conn = sqlite3.connect(str(db_path))
        conn.execute("CREATE TABLE neurons (id TEXT)")
        conn.execute("CREATE TABLE synapses (id TEXT, source_id TEXT, target_id TEXT, type TEXT, weight REAL)")
    ...
    async def test_wrong_embedding_dimensions(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
...
class TestMigratorAlternateSchema:
    ...
    async def test_synapse_alt_columns(self, tmp_path: Path, infinity_db: InfinityDB) -> None:
        ...
        conn.execute("INSERT INTO synapses (synapse_id, source_id, target_id, synapse_type) VALUES ('s1', 'n1', 'n2', 'causal')")
        conn.commit()
        conn.close()
...
class TestMigrationIntegration:
    async def test_migrate_then_search(self, tmp_path: Path) -> None:
        ...
        db_path = _create_sqlite_db(
            tmp_path / "src.db", neurons=20, embeddings=True, dimensions=8
        )
    ...
    async def test_migrate_then_graph_traverse(self, tmp_path: Path) -> None:
        ...
        db_path = _create_sqlite_db(
            tmp_path / "src.db", neurons=10, synapses=9
        )
        db = InfinityDB(tmp_path / "infinity", dimensions=8)
        await db.open()
    ...
    async def test_migrate_then_fiber_lookup(self, tmp_path: Path) -> None:
        ...
        db_path = _create_sqlite_db(
            tmp_path / "src.db", neurons=5, fibers=3
        )
        db = InfinityDB(tmp_path / "infinity", dimensions=8)
        await db.open()
    ...
    async def test_idempotent_migration(self, tmp_path: Path) -> None:
        ...
        db_path = _create_sqlite_db(
            tmp_path / "src.db", neurons=5, synapses=3, fibers=2
        )
        db = InfinityDB(tmp_path / "infinity", dimensions=8)
        await db.open()
