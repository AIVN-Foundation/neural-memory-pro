...
from neural_memory_pro.consolidation.smart_merge import MergeAction, smart_merge
from neural_memory_pro.hyperspace.directional_compress import (
...
from neural_memory_pro.retrieval.cone_queries import ConeResult, cone_recall
...
class TestConeQueries:
    ...
    async def test_cone_result_dataclass(self) -> None:
        r = ConeResult(
            neuron_id="n1", content="test", similarity=0.95,
            activation=0.7, combined_score=0.88, neuron_type="fact",
        )
...
class TestSmartMerge:
    ...
    async def test_merge_similar_neurons(self, db_dir: Path) -> None:
        ...
        await db.add_neuron("Memory about cats", neuron_id="n1",
                           embedding=base_vec, priority=8)
        await db.add_neuron("Also about cats and dogs", neuron_id="n2",
                           embedding=_similar_vec(base_vec, 0.01), priority=5)
        await db.add_neuron("Cat behavior patterns", neuron_id="n3",
                           embedding=_similar_vec(base_vec, 0.02), priority=4)

        # Dissimilar neuron — should not merge
        await db.add_neuron("About physics", neuron_id="n4",
                           embedding=_make_vec(999), priority=5)
    ...
    async def test_merge_executes(self, db_dir: Path) -> None:
        ...
        await db.add_neuron("Primary content", neuron_id="n1",
                           embedding=base_vec, priority=9)
        await db.add_neuron("Secondary content", neuron_id="n2",
                           embedding=_similar_vec(base_vec, 0.01), priority=3)
    ...
    async def test_no_similar_neurons(self, db_dir: Path) -> None:
        ...
        # All dissimilar
        for i in range(5):
            await db.add_neuron(f"topic {i}", neuron_id=f"n{i}",
                               embedding=_make_vec(i * 100))

        result = await smart_merge(db, similarity_threshold=0.99)
        assert result["merges"] == 0 or result["status"] in ("no_clusters", "insufficient_embeddings")
        await db.close()
    ...
    async def test_dry_run_does_not_modify(self, db_dir: Path) -> None:
        db = await _setup_db(db_dir)
        base_vec = _make_vec(1)
        await db.add_neuron("Original A", neuron_id="n1",
                           embedding=base_vec, priority=8)
        await db.add_neuron("Original B", neuron_id="n2",
                           embedding=_similar_vec(base_vec, 0.01), priority=3)
    ...
    def test_merge_action_dataclass(self) -> None:
        action = MergeAction(
            anchor_id="a1", merged_ids=("m1", "m2"),
            new_content="merged", reason="test",
        )
